import { locale, register, isLoading } from 'svelte-i18n'
...
// register languages
Object.entries(LOCALE_IMPORTS).forEach(([locale, importFunction]) =>
  register(locale, importFunction)
)
...
const handleLocaleChange = serverStore => newLocale => {
  if (newLocale) {
    ...
    const basePath =
      (!IS_SERVER_SIDE && location.pathname) || get(serverStore).pathname
...
export const isLoadingLocale = derived(
  [isLoading, locale],
  ([$isLoading, $locale]) => {
