...
const MAPKIT_TOKEN_REGEX = /api\/jwt\/mapkit\-token/

// `shell` is an array of all the files generated by the bundler,
// `files` is an array of everything in the `static` directory
const to_cache = shell
  .concat(files)
  .filter(src => src.substr(src.lastIndexOf('.') + 1) !== 'js')
...
async function fetchAndCache(request, url) {
  // MapKit JS is pretty resource heavy and gains little from caching so let's avoid it
  const isMapkitResource = url.hostname.match(MAPKIT_RESOURCE_REGEX)
  // We want a new token every time
  const isMapkitTokenRequest = url.pathname.match(MAPKIT_TOKEN_REGEX)
  const shouldCache = !isMapkitResource && !isMapkitTokenRequest
